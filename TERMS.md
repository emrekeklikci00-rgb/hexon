# Terms of Service - Hexon Bot

Last updated: 2024

## 1. Acceptance
By using Hexon Bot, you agree to these Terms of Service.

## 2. Use of the Bot
- You must follow Discord's Terms of Service
- Do not use the bot for illegal activities
- Do not spam or abuse bot commands

## 3. Data Collection
We collect minimal data necessary for bot functionality:
- User IDs (for economy and level systems)
- Server IDs (for configuration)
- Message counts (for level system only)

## 4. Liability
The bot is provided "as is" without warranties.

## 5. Changes
We may update these terms at any time.

Contact: [em4eofficial]
